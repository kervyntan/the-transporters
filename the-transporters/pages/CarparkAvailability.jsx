...
import { carparkAvailability } from "../store/TransportStore";
...
const CarparkAvailability = () => {
  ...
  useEffect(() => {
    carparkAvailability().then((res) => {
        setResult(res.data)
    });
  }, []);
  return (
    <View
      style={{ backgroundColor: Colours.primaryLite, flex: 1, padding: 15 }}
    >
      <View>
      ...
      <View style={carparkStyle.searchContainer}>
        ...
        <TouchableOpacity style={carparkStyle.searchBtn} onPress={() => {}}>
          <Image
      ...
      <View style={carparkStyle.tabsContainer}>
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Text style={carparkStyle.tabText(activeFilter, item)}>
                {" "}
                {item}{" "}
              </Text>
            </TouchableOpacity>
